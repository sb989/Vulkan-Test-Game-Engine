...
namespace image{
        VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels){
            ...
            if (vkCreateImageView(*sharedVariables::device, &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
        ...
        void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, 
         ...
         VkImage& image, VkDeviceMemory& imageMemory){
            ...
            imageInfo.flags = 0;
            if(vkCreateImage(*sharedVariables::device, &imageInfo, nullptr, &image) != VK_SUCCESS){
                throw std::runtime_error("failed to create image!");
            }
            VkMemoryRequirements memRequirements;
            vkGetImageMemoryRequirements(*sharedVariables::device, image, &memRequirements);
            ...
            if(vkAllocateMemory(*sharedVariables::device, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS){
                throw std::runtime_error("failed to allocate image memory!");
            }
            vkBindImageMemory(*sharedVariables::device, image, imageMemory, 0);
        }
